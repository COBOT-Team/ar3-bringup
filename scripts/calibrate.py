...
INIT_MSG = frame(b'\x00' + struct.pack('<I', 0) + struct.pack('<I', FIRMWARE_VERSION))
CALIBRATE_MSG = frame(b'\x01' + struct.pack('<I', 1) + struct.pack('B', 0b111111))
...
def wait_for_ack(ser, response_id):
    response_type, response_id, msg = wait_for_response(ser)
    if response_type != 0x00:
        raise Exception(f'Expected ACK, got {response_type}')
    if response_id != response_id:
        raise Exception(f'Expected ACK for message {response_id}, got {response_id}')
...
def wait_for_done(ser, response_id):
    ...
    if response_id != response_id:
        raise Exception(f'Expected DONE for message {response_id}, got {response_id}')
    return msg[0]
